...
main(int argc, char* argv[]) {
  ...
  SDL_Window *sdlWindow = SDL_CreateWindow("OpenGL Test", 0, 0, WinWidth, WinHeight, sdlWindowFlags);
  if (!sdlWindow) {
      ...
      if (Event.type == SDL_KEYDOWN) {
        switch (Event.key.keysym.sym) {
          ...
          case 'f':
            ...
            if (fullScreen) {
              SDL_SetWindowFullscreen(sdlWindow, sdlWindowFlags | SDL_WINDOW_FULLSCREEN_DESKTOP);
            }
            else {
